...
export function Card({ data }: CardProps) {
	return (
		<UiLink
			...
			className='block p-3 relative w-full 2xl:h-full'
		>
			<UiImage
				...
				loading={'eager'}
				classNames={{
					img: '!h-[150px] !2xl:h-full object-cover object-center rounded-sm overflow-hidden',
				}}
			/>
			<div className={'flex flex-col justify-between 2xl:absolute bottom-[10px] left-[10px] z-3'}>
				<div className={'flex flex-wrap items-center gap-2 mt-[8px]'}>
					{data.categories.map((category) => (
						<div key={category.id} className={'flex items-center gap-1'}>
							...
							<div className={'text-xs text-black 2xl:text-white whitespace-nowrap'}>
				...
				<h3
					className={
						'mt-3 text-md text-back 2xl:text-white font-medium hover:text-[#fe4444] transition-colors duration-300 line-clamp-2'
					}
				>
			...
			<div
				className={'hidden 2xl:block absolute top-0 left-0 w-full h-full z-2'}
				style={{
